...
class LobbyScene : public CCLayer ,public CCTextFieldDelegate
{
public:
...
bool LobbyScene::init()
{
    ...
    CCMenuItemImage *pCloseItem = CCMenuItemImage::create(
                                        ...
                                        "CloseSelected.png",
                                        this,
                                        menu_selector(LobbyScene::menuCloseCallback));

	pCloseItem->setPosition(ccp(size.width - pCloseItem->getContentSize().width/2 ,pCloseItem->getContentSize().height/2));


	CCMenuItemImage *loginButton = CCMenuItemImage::create("image/c8.png","image/c3.png",this,menu_selector(LobbyScene::loginButtonClicked));
	...
	CCMenuItemImage *registButton = CCMenuItemImage::create("image/c8.png","image/c3.png",this,menu_selector(LobbyScene::registButtonClicked));
	...
	CCMenuItem* tapItem1 = CCMenuItemFont::create("              ",this,menu_selector(LobbyScene::textFieldPressed1));
	...
	tapItem2 = CCMenuItemFont::create("              ",this,menu_selector(LobbyScene::textFieldPressed2));
	...
	CCMenuItem* tapItem3 = CCMenuItemFont::create("              ",this,menu_selector(LobbyScene::textFieldPressed3));
...
void LobbyScene::textFieldPressed1(CCObject *sender)
{
	text1->attachWithIME();
}
void LobbyScene::textFieldPressed3(CCObject *sender)
{
	text3->attachWithIME();
}
void LobbyScene::textFieldPressed2(CCObject *sender)
{
	text2->attachWithIME();
	text2->setVisible(false);
	tapItem2->setString("********");
}
bool LobbyScene::onTextFieldAttachWithIME(CCTextFieldTTF *sender)
{
	SimpleAudioEngine::sharedEngine()->playEffect("audio/ef_0.ogg");
	return false;
}
bool LobbyScene::onTextFieldDetachWithIME(CCTextFieldTTF *sender)
{
	SimpleAudioEngine::sharedEngine()->playEffect("audio/ef_0.ogg");
	return false;
}
void LobbyScene::checkNameExist(){
	CURL *curl;
	CURLcode res;
	char buffer[10];
	curl = curl_easy_init();
...
void LobbyScene::loginButtonClicked(CCObject* pSender)
{
	SimpleAudioEngine::sharedEngine()->playEffect("audio/ef_0.ogg");
	checkNameExist();
	if(httpAns!=-404){
...
void LobbyScene::registButtonClicked(CCObject* pSender)
{
	SimpleAudioEngine::sharedEngine()->playEffect("audio/ef_0.ogg");
	checkNameExist();
	if(httpAns!=-404){
...
size_t LobbyScene::writehtml(uint8_t* ptr,size_t size,size_t number,void *stream){
	char ans[4];
...
void LobbyScene::menuCloseCallback(CCObject* pSender)
{
	SimpleAudioEngine::sharedEngine()->playEffect("audio/ef_0.ogg");
